...
- `-p` / `--profile` now accepts arbitrary profile names — `-p cheap` resolves to `<cwd>/.council/cheap.yaml` first, then `~/.config/council/cheap.yaml`. Embedded fallback is reserved for `-p default` so a typo'd non-default name is a hard error rather than a silent run against the embedded profile. Names must match `^[a-zA-Z0-9][a-zA-Z0-9_-]*$`. The flag also accepts a path: any value containing `/` or ending in `.yaml` is loaded directly as an explicit YAML file. Use case: keep a `cheap.yaml` (haiku / gpt-mini) alongside the default `default.yaml` and switch with one flag. Resolution is exposed as `pkg/config.LoadByName(cwd, name)`; the previous `pkg/config.Load(cwd)` entry point is removed (single-user codebase, no compat shim).
