...
| F27 | Gemini writes policy + emits `--policy` when PermissionMode is bypassPermissions (ADR-0012) | `PermissionMode="bypassPermissions"` → `$GEMINI_CLI_HOME/policy.toml` exists with the embedded allow-rule body; argv contains `--policy <that path>`. Empty PermissionMode → no policy file, no `--policy` flag, no `--allowed-tools`, no `--yolo` |
...
| F33 | Quorum absorbs single rate-limit (ADR-0013) | 3-expert profile, one mocked → `*LimitError`; quorum=2; verdict written with `status: success` and `rate_limits` populated |
