...
**Why:** Session-local cache would add folder layout, schema fields,
and prompt scaffolding ("Peer A fetched these URLs: [...]; read them
from `./fetches/` before re-fetching") for small gains at v2 scale.
YAGNI.

**Alternatives:** Session-local cache considered (ADR-0010 alt d);
deferred to v3 if token costs bite.

**Research:** None directly; an engineering simplicity call.
