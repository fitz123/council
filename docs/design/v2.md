...
(hardcoded in the claudecode executor — see D17). Experts are prompted
(see the R1 body in `defaults/prompts/independent.md`) to use tools
when facts matter and cite URLs they fetched. See ADR-0010 for the
full decision and research backing.
...
available. Hardcoded in the `claudecode` executor — no profile
`tools:` block, no CLI flag, no environment variable. Ballot
subprocesses still hardcoded tools-off (internal rule in `pkg/debate/vote.go`).
Full decision in ADR-0010.
